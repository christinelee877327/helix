...
function openDb(name, version = undefined) {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = function (event) {
            if (name === 'my-export-db' && wasCreated) {
...
function toJsonSafe_Host(value, name) {
    const hostExport = {
        id: value?.id?._serialized || value?.id || value?.__x_id?._serialized,
        name: name ?? value?.pushname,
...
async function getHostInfo() {
    ...
    try {
        ...
        const id = hostInfo.id._serialized
        const maxItem = Object.values(e.getDisplayName.$$cache.get(id))
            .filter(v => v && typeof v === 'object' && 'type' in v)
            .reduce((max, cur) => {
                const curType = cur.type ?? -Infinity;
                const maxType = max?.type ?? -Infinity;
                return curType > maxType ? cur : max;
            }, null);
        const sanitizedHostInfo = toJsonSafe_Host(hostInfo, maxItem?.displayName)
...
async function processChats() {
    ...
    console.log("実行前、全部書き込みmodule", !globalThis.MY_EXPORT_DB_WAS_CREATED)
    if (!exportDbExists) {
